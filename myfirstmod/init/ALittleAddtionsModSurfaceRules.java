...
import net.mcreator.myfirstmod.mixins.NoiseGeneratorSettingsAccess;

@Mod.EventBusSubscriber
public class ALittleAddtionsModSurfaceRules {
	@SubscribeEvent
	public static void init(ServerAboutToStartEvent event) {
		...
		boolean hasEndBiomes = chunkGenerator.getBiomeSource().possibleBiomes().stream().anyMatch(biomeHolder -> biomeHolder.unwrapKey().orElseThrow().location().getNamespace().equals("a_little_addtions"));
		if (hasEndBiomes) {
			if (chunkGenerator instanceof NoiseBasedChunkGenerator generator) {
				...
				registerSurfaceRules(new ResourceLocation("a_little_addtions:endexpansion"), noiseGeneratorSettings, ALittleAddtionsModBlocks.ENDGRASS.get().defaultBlockState(), ALittleAddtionsModBlocks.ENDSOIL.get().defaultBlockState());
				registerSurfaceRules(new ResourceLocation("a_little_addtions:dupe"), noiseGeneratorSettings, ALittleAddtionsModBlocks.ENDGRASS.get().defaultBlockState(), Blocks.END_STONE.defaultBlockState());
			}
		}
	}
